...
int unscaled_bias[THREE_AXES * 2]; // debug
...
inv_sensor_event_t gstImuEvent;
...
void build_sensor_event_data(void * context, uint8_t sensortype, uint64_t timestamp, const void * data, const void *arg){
	...
	gstImuEvent.sensor = sensor_id;
	gstImuEvent.timestamp = timestamp;
	switch(sensor_id) {
	case INV_SENSOR_TYPE_UNCAL_GYROSCOPE:
		memcpy(raw_bias_data, data, sizeof(raw_bias_data));
		memcpy(gstImuEvent.data.gyr.vect, &raw_bias_data[0], sizeof(gstImuEvent.data.gyr.vect));
		memcpy(gstImuEvent.data.gyr.bias, &raw_bias_data[3], sizeof(gstImuEvent.data.gyr.bias));
		memcpy(&(gstImuEvent.data.gyr.accuracy_flag), arg, sizeof(gstImuEvent.data.gyr.accuracy_flag));
		break;
	case INV_SENSOR_TYPE_UNCAL_MAGNETOMETER:
		memcpy(raw_bias_data, data, sizeof(raw_bias_data));
		memcpy(gstImuEvent.data.mag.vect, &raw_bias_data[0], sizeof(gstImuEvent.data.mag.vect));
		memcpy(gstImuEvent.data.mag.bias, &raw_bias_data[3], sizeof(gstImuEvent.data.mag.bias));
		memcpy(&(gstImuEvent.data.gyr.accuracy_flag), arg, sizeof(gstImuEvent.data.gyr.accuracy_flag));
		break;
	case INV_SENSOR_TYPE_GYROSCOPE:
		memcpy(gstImuEvent.data.gyr.vect, data, sizeof(gstImuEvent.data.gyr.vect));
		memcpy(&(gstImuEvent.data.gyr.accuracy_flag), arg, sizeof(gstImuEvent.data.gyr.accuracy_flag));
		//INV_MSG(INV_MSG_LEVEL_DEBUG, "Gyro[%02X] = %2.8f | %2.8f | %2.8f ", gstImuEvent.data.gyr.accuracy_flag, gstImuEvent.data.gyr.vect[0], gstImuEvent.data.gyr.vect[1], gstImuEvent.data.gyr.vect[2]);
		break;
	case INV_SENSOR_TYPE_GRAVITY:
		memcpy(gstImuEvent.data.acc.vect, data, sizeof(gstImuEvent.data.acc.vect));
		gstImuEvent.data.acc.accuracy_flag = inv_icm20948_get_accel_accuracy();
		break;
	case INV_SENSOR_TYPE_LINEAR_ACCELERATION:
	case INV_SENSOR_TYPE_ACCELEROMETER:
		memcpy(gstImuEvent.data.acc.vect, data, sizeof(gstImuEvent.data.acc.vect));
		memcpy(&(gstImuEvent.data.acc.accuracy_flag), arg, sizeof(gstImuEvent.data.acc.accuracy_flag));
		break;
	case INV_SENSOR_TYPE_MAGNETOMETER:
		memcpy(gstImuEvent.data.mag.vect, data, sizeof(gstImuEvent.data.mag.vect));
		memcpy(&(gstImuEvent.data.mag.accuracy_flag), arg, sizeof(gstImuEvent.data.mag.accuracy_flag));
		break;
	case INV_SENSOR_TYPE_GEOMAG_ROTATION_VECTOR:
	case INV_SENSOR_TYPE_ROTATION_VECTOR:
		memcpy(&(gstImuEvent.data.quaternion.accuracy), arg, sizeof(gstImuEvent.data.quaternion.accuracy));
		memcpy(gstImuEvent.data.quaternion.quat, data, sizeof(gstImuEvent.data.quaternion.quat));
		//INV_MSG(INV_MSG_LEVEL_DEBUG, "Quat[%02X] = %2.8f | %2.8f | %2.8f | %2.8f", gstImuEvent.data.quaternion.accuracy, gstImuEvent.data.quaternion.quat[0],
		//																			gstImuEvent.data.quaternion.quat[1], gstImuEvent.data.quaternion.quat[2],
		//																			gstImuEvent.data.quaternion.quat[3]);
		
		break;
	case INV_SENSOR_TYPE_GAME_ROTATION_VECTOR:
		memcpy(gstImuEvent.data.quaternion.quat, data, sizeof(gstImuEvent.data.quaternion.quat));
		gstImuEvent.data.quaternion.accuracy_flag = icm20948_get_grv_accuracy();
		break;
	case INV_SENSOR_TYPE_BAC:
		memcpy(&(gstImuEvent.data.bac.event), data, sizeof(gstImuEvent.data.bac.event));
		break;
	case INV_SENSOR_TYPE_PICK_UP_GESTURE:
	case INV_SENSOR_TYPE_TILT_DETECTOR:
	case INV_SENSOR_TYPE_STEP_DETECTOR:
	case INV_SENSOR_TYPE_SMD:
		gstImuEvent.data.event = true;
		break;
	case INV_SENSOR_TYPE_B2S:
		gstImuEvent.data.event = true;
		memcpy(&(gstImuEvent.data.b2s.direction), data, sizeof(gstImuEvent.data.b2s.direction));
		break;
	case INV_SENSOR_TYPE_STEP_COUNTER:
		memcpy(&(gstImuEvent.data.step.count), data, sizeof(gstImuEvent.data.step.count));
		break;
	case INV_SENSOR_TYPE_ORIENTATION:
		//we just want to copy x,y,z from orientation data
		memcpy(&(gstImuEvent.data.orientation), data, 3*sizeof(float));
	...
	case INV_SENSOR_TYPE_RAW_GYROSCOPE:
		memcpy(gstImuEvent.data.raw3d.vect, data, sizeof(gstImuEvent.data.raw3d.vect));
		break;
	default:
		return;
	}
...
//
//		/* Clear pio interrupt */
////		pio_clear(PIN_EXT_INTERRUPT_PIO, PIN_EXT_INTERRUPT_MASK); // 2020-06-11
//
//		/* Re-init the device */
...
int userSetSensorPeriod(int sensor_id, uint32_t uiPeriod)
{
	...
	INV_MSG(INV_MSG_LEVEL_DEBUG, "DeviceEmdWrapper: received command set_period(%d us)",uiPeriod);
	rc = inv_icm20948_set_sensor_period(&icm_device, idd_sensortype_conversion(sensor), uiPeriod / 1000);
	return rc;
}
